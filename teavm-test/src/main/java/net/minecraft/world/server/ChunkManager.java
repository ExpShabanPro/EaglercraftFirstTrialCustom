...
//             }).filter(this::chunkSave).forEach((p_222959_1_) -> {
...
//                   } else {
//                      completablefuture1 = chunkStatus.doLoadingWork(this.world, this.templateManager, this.lightManager, (p_223175_2_) -> {
...
//       ChunkPos chunkpos = chunkHolder.getPosition();
//       CompletableFuture<Either<List<IChunk>, ChunkHolder.IChunkLoadingError>> completablefuture = this.func_219236_a(chunkpos, chunkStatus.getTaskRange(), (p_219195_2_) -> {
//          return this.func_219205_a(chunkStatus, p_219195_2_);
//       });
...
//             try {
//                CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>> completablefuture1 = chunkStatus.doGenerationWork(this.world, this.generator, this.templateManager, this.lightManager, (p_222954_2_) -> {
//                   return this.func_219200_b(chunkHolder);
//                }, p_223148_4_);
//                this.field_219266_t.statusChanged(chunkpos, chunkStatus);
//                return completablefuture1;
...
//          });
//       }, (p_219216_2_) -> {
//          this.field_219264_r.enqueue(ChunkTaskPriorityQueueSorter.func_219081_a(chunkHolder, p_219216_2_));
//       });
//    }
...
//                chunk = ((ChunkPrimerWrapper)p_219237_2_).getChunk();
//             } else {
...
import net.minecraft.util.math.SectionPos;
import net.minecraft.util.math.vector.Vector3d;
import net.minecraft.world.ChunkStatus;
import net.minecraft.world.GameRules;
import net.minecraft.world.chunk.Chunk;
import net.minecraft.world.chunk.ChunkPrimer;
import net.minecraft.world.chunk.ChunkPrimerWrapper;
...
public class ChunkManager extends ChunkLoader implements ChunkHolder.IPlayerProvider {
   private static final Logger LOGGER = LogManager.getLogger();
   public static final int MAX_LOADED_LEVEL = 33 + ChunkStatus.maxDistance();
   private final Long2ObjectLinkedOpenHashMap<ChunkHolder> field_219248_f = new Long2ObjectLinkedOpenHashMap<>();
   private volatile Long2ObjectLinkedOpenHashMap<ChunkHolder> field_219249_g = this.field_219248_f.clone();
   private final Long2ObjectLinkedOpenHashMap<ChunkHolder> unloadableChunks = new Long2ObjectLinkedOpenHashMap<>();
   private final LongSet field_219251_i = new LongOpenHashSet();
   private final ServerWorld world;
   private final WorldLightManager lightManager;
   private final ThreadTaskExecutor<Runnable> mainThreadExecutor;
   private final ChunkGenerator generator;
   private final Supplier<DimensionSavedDataManager> field_219256_n;
   private final IChunkStatusListener field_219257_o;
   private final ChunkManager.ProxyTicketManager ticketManager;
   private int totalPendingForSave;
   private final DataFixer field_219259_q;
   private final File field_219260_r;
   private final PlayerChunkMap playerChunkMap = new PlayerChunkMap();
   private final Int2ObjectMap<ChunkManager.EntityTracker> entities = new Int2ObjectOpenHashMap<>();
   private final Long2ByteMap chunkViewDistances = new Long2ByteOpenHashMap();
   private final Queue<Runnable> field_219264_v = Queues.newConcurrentLinkedQueue();
   private final IOWorker field_227092_y_;

   public ChunkManager(ServerWorld world, File worldDir, DataFixer dataFixer, StructureTemplateManager templateManager, Executor p_i51507_5_, ThreadTaskExecutor<Runnable> mainThread, IChunkLightProvider lightProvider, ChunkGenerator generator, IChunkStatusListener statusListener, Supplier<DimensionSavedDataManager> p_i51507_10_, int viewDistance, boolean p_i51507_12_) {
      super(new File(worldDir, "region"), dataFixer, p_i51507_12_);
      this.field_219259_q = dataFixer;
      this.field_219260_r = worldDir;
      this.world = world;
      this.generator = generator;
      this.mainThreadExecutor = mainThread;
      
      this.field_227092_y_ = new IOWorker(new File(worldDir, "region"), p_i51507_12_, "chunk");
      this.lightManager = new WorldLightManager(lightProvider, this, world.getDimensionType().hasSkyLight());
      this.field_219257_o = statusListener;
      this.field_219256_n = p_i51507_10_;
      this.ticketManager = new ChunkManager.ProxyTicketManager(p_i51507_5_, mainThread);
   ...
   protected ChunkHolder func_219219_b(long chunkPos) {
      return this.field_219248_f.get(chunkPos);
   }

   protected ChunkHolder func_219220_a(long chunkPos) {
      return this.field_219249_g.get(chunkPos);
   }

   protected CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>> func_219244_a(ChunkHolder holder, ChunkStatus status) {
      ChunkPos chunkpos = holder.getPosition();
      if (status == ChunkStatus.EMPTY) {
         return this.func_219181_a(chunkpos);
      } else {
         CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>> completablefuture = holder.func_219301_a(status.getParent());
         return completablefuture.thenComposeAsync((either) -> {
            return either.map((chunk) -> {
               try {
                  CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>> cf = status.func_223222_a(this.world, this.generator, this.field_219256_n.get(), (chunks) -> {
                     this.func_219215_a(chunkpos, chunks);
                  }, chunk);
                  this.field_219257_o.onChunkStatusChange(chunkpos, status);
                  return cf;
               } catch (Exception exception) {
                  CrashReport crashreport = CrashReport.makeCrashReport(exception, "Chunk loading");
                  CrashReportCategory crashreportcategory = crashreport.makeCategory("Chunk being loaded");
                  crashreportcategory.addDetail("Worker to load", status.getName());
                  crashreportcategory.addDetail("Full (requested) status", status.getName());
                  crashreportcategory.addDetail("Chunk pos", chunkpos.toString());
                  throw new ReportedException(crashreport);
               }
            }, (err) -> {
               return CompletableFuture.completedFuture(Either.right(err));
            });
         }, this.mainThreadExecutor);
      }
   }

   private CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>> func_219181_a(ChunkPos pos) {
      return CompletableFuture.supplyAsync(() -> {
         try {
            IChunk ichunk = this.func_219226_a(pos);
            if (ichunk != null) {
               return Either.left(ichunk);
            }
         } catch (Exception exception) {
            LOGGER.error("Couldn't load chunk {}", pos, exception);
         }
         return Either.left(new ChunkPrimer(pos, UpgradeData.EMPTY));
      }, this.mainThreadExecutor);
   }

   public CompletableFuture<Either<Chunk, ChunkHolder.IChunkLoadingError>> func_222961_b(ChunkHolder holder) {
      return holder.func_219301_a(ChunkStatus.FULL).thenApplyAsync((either) -> {
   ...
   public CompletableFuture<Either<Chunk, ChunkHolder.IChunkLoadingError>> func_219179_a(ChunkHolder holder) {
      return holder.getBorderFuture().thenApplyAsync((either) -> {
         return either.mapLeft((chunk) -> {
            this.world.func_217447_a(chunk);
            return chunk;
         });
      }, this.mainThreadExecutor);
   }
   ...
   public CompletableFuture<Either<Chunk, ChunkHolder.IChunkLoadingError>> func_219188_b(ChunkPos pos) {
      return this.func_219244_a(this.func_219220_a(pos.asLong()), ChunkStatus.FULL).thenApplyAsync((either) -> {
         return either.mapLeft((chunk) -> {
            return (Chunk)chunk;
         });
      }, this.mainThreadExecutor);
   ...
   private void func_219215_a(ChunkPos pos, List<IChunk> chunks) {
      // Logic for status listener or tracking
   }

   public void setViewDistance(int distance) {
      int i = MathHelper.clamp(distance + 1, 3, 33);
      if (i != this.playerChunkMap.func_219114_c()) {
         this.playerChunkMap.func_219114_c();
      }
   }

   public Stream<ServerPlayerEntity> getTrackingPlayers(ChunkPos pos, boolean boundaryOnly) {
      return this.playerChunkMap.getTrackingPlayers(pos.asLong(), boundaryOnly);
   }

   protected void track(Entity entityIn) {
      if (!(entityIn instanceof net.minecraft.entity.projectile.EyeOfEnderEntity)) {
         EntityType<?> entitytype = entityIn.getType();
         int i = entitytype.func_233602_m_() * 16;
         int j = entitytype.func_223611_l();
         if (this.entities.containsKey(entityIn.getEntityId())) {
            throw (IllegalStateException)Util.pauseDevMode(new IllegalStateException("Entity is already tracked!"));
         } else {
            ChunkManager.EntityTracker chunkmanager$entitytracker = new ChunkManager.EntityTracker(entityIn, i, j, entitytype.func_223610_k());
            this.entities.put(entityIn.getEntityId(), chunkmanager$entitytracker);
            chunkmanager$entitytracker.updateTrackingState(this.world.getPlayers());
            if (entityIn instanceof ServerPlayerEntity) {
               ServerPlayerEntity serverplayerentity = (ServerPlayerEntity)entityIn;
               this.func_219209_a(serverplayerentity);
   ...
   protected void untrack(Entity entityIn) {
      if (entityIn instanceof ServerPlayerEntity) {
         ServerPlayerEntity serverplayerentity = (ServerPlayerEntity)entityIn;
         this.func_219230_b(serverplayerentity);

         for(ChunkManager.EntityTracker chunkmanager$entitytracker : this.entities.values()) {
            chunkmanager$entitytracker.untrack(serverplayerentity);
         }
      }

      ChunkManager.EntityTracker chunkmanager$entitytracker1 = this.entities.remove(entityIn.getEntityId());
      if (chunkmanager$entitytracker1 != null) {
         chunkmanager$entitytracker1.removeAllTrackedPlayers();
      }

   }

   protected void tick() {
      List<ServerPlayerEntity> list = Lists.newArrayList(this.world.getPlayers());
      List<ChunkManager.EntityTracker> trackers = Lists.newArrayList(this.entities.values());
      for(ChunkManager.EntityTracker tracker : trackers) {
         tracker.updateTrackingState(list);
      }
   }

   private void func_219209_a(ServerPlayerEntity player) {
      // Player tracking init
   }

   private void func_219230_b(ServerPlayerEntity player) {
   ...
   class EntityTracker {
      ...
      public EntityTracker(Entity entity, int range, int freq, boolean velocity) {
         this.entity = entity;
         this.range = range;
         this.updateFrequency = freq;
         this.sendVelocityUpdates = velocity;
      }

      public void updateTrackingState(List<ServerPlayerEntity> players) {
         for(ServerPlayerEntity player : players) {
            this.updateTrackingState(player);
         }
      }

      public void updateTrackingState(ServerPlayerEntity player) {
         if (player != this.entity) {
            Vector3d vector3d = player.getPositionVec().subtract(this.entity.getPositionVec());
            int i = Math.min(this.range, ChunkManager.this.world.getGameRules().getInt(GameRules.field_234890_J_) * 16);
            boolean flag = vector3d.x >= (double)(-i) && vector3d.x <= (double)i && vector3d.z >= (double)(-i) && vector3d.z <= (double)i && this.entity.isSpectator();
            if (flag) {
               if (this.trackingPlayers.add(player)) {
                  this.entity.addTrackingPlayer(player);
                  player.connection.sendPacket(this.entity.getSpawnPacket());
               }
            } else if (this.trackingPlayers.remove(player)) {
               this.entity.removeTrackingPlayer(player);
               player.connection.sendPacket(new net.minecraft.network.play.server.SDestroyEntitiesPacket(this.entity.getEntityId()));
            }
         }
      }
      ...
      public void removeAllTrackedPlayers() {
   ...
   class ProxyTicketManager extends TicketManager {
      ...
      protected ChunkHolder getChunkHolder(long pos) {
         return ChunkManager.this.func_219219_b(pos);
      }

      protected ChunkHolder setChunkLevel(long pos, int level, ChunkHolder holder, int p_219372_5_) {
         return ChunkManager.this.setChunkLevel(pos, level, holder, p_219372_5_);
      }
   }

   private ChunkHolder setChunkLevel(long pos, int level, ChunkHolder holder, int p_219372_5_) {
      // Core chunk level logic
      return holder;
   }
}
